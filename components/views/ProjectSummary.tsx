...
import { PricingRepository, ResourceConfig, ServiceType } from '../../types';
...
export const ProjectSummary: React.FC<ProjectSummaryProps> = ({ resources, onRemove, onEdit, pricingData }) => {
  const { totalMonthly, totalAnnual, resourceCosts } = useMemo(() => {
    let monthly = 0;
    if (!pricingData) return { totalMonthly: 0, totalAnnual: 0, resourceCosts: [] };

    const itemCosts = resources.map(res => {
      const cost = calculateResourceCost(res, pricingData);
