...
import { ResourceConfig, ApiGatewayType, ApiGatewayCacheSize, ServiceType } from '../../types';
...
export const ApiGatewayArchitectureConfig: React.FC<ApiGatewayModuleProps> = ({ config, onUpdate }) => {
  ...
  const cacheSizes = getServiceOptions(ServiceType.API_GATEWAY, 'cache');
                         ...
                         {attrs.apiGwCacheEnabled && (
                            <div className="animate-in fade-in slide-in-from-top-2">
                                <FormSection label="Cache Capacity">
                                    ...
                                    >
                                        {cacheSizes.length > 0 ? (
                                            cacheSizes.map(s => <option key={s} value={s}>{s} GB</option>)
                                        ) : (
                                            Object.values(ApiGatewayCacheSize).map(s => <option key={s} value={s}>{s}</option>)
                                        )}
                                    </AWSSelect>
                                </FormSection>
                            </div>
