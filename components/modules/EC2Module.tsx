...
import { ResourceConfig, InstanceType, InstanceFamily, Tenancy, EbsVolumeType, LoadBalancerType } from '../../types';
...
export const EC2ArchitectureConfig: React.FC<EC2ModuleProps> = ({ config, onUpdate, availableVpcs = [] }) => {
  ...
  // Initialize view mode based on current count. 
  // If count is 1, default to Single view, otherwise Fleet view.
  ...
  const handleModeSwitch = (fleetMode: boolean) => {
      ...
      if (!fleetMode) {
          ...
          // If switching to fleet and count is 1, suggest 2 to show difference, otherwise keep current
        ...
        <Accordion title="Instance Configuration" defaultOpen={true}>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <FormSection 
                        label="Instance Type"
                        tooltip="Specific hardware combination of vCPU, Memory, and Networking."
                    >
                        <AWSSelect 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        />
                         <AWSInput 
                            ...
                            disabled={attrs.rootVolumeType !== EbsVolumeType.IO2 && attrs.rootVolumeType !== EbsVolumeType.GP3}
                    ...
                    >
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <AWSInput 
                                ...
                                type="number" min={0}
                                value={attrs.dataVolumeCount}
                                onChange={(e) => updateAttribute('dataVolumeCount', Number(e.target.value))}
                            />
                             <AWSSelect 
                                label="Volume Type"
                                value={attrs.dataVolumeType}
                                onChange={(e) => updateAttribute('dataVolumeType', e.target.value)}
                            >
                                {Object.values(EbsVolumeType).map(t => (
                                    <option key={t} value={t}>{t}</option>
                                ))}
                         ...
                         {attrs.dataVolumeCount > 0 && (
                             <AWSInput 
                                label="Volume Size (Each)"
                                type="number" min={1}
                                value={attrs.dataVolumeSize}
                                onChange={(e) => updateAttribute('dataVolumeSize', Number(e.target.value))}
                                unit="GB"
                            />
                         )}
                    </FormSection>
