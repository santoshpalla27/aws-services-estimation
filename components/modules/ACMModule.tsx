import React from 'react';
import { Accordion, FormSection, AWSSlider, AWSInput, AWSSelect, AWSButton } from '../ui/AWS';
import { ResourceConfig, AcmCertificateType } from '../../types';

interface ACMModuleProps {
  config: ResourceConfig;
  onUpdate: (newConfig: ResourceConfig) => void;
}

export const ACMArchitectureConfig: React.FC<ACMModuleProps> = ({ config, onUpdate }) => {
  const attrs = config.attributes;

  const updateAttribute = (key: string, value: any) => {
    onUpdate({
        ...config,
        attributes: { ...attrs, [key]: value }
    });
  };

  const addCert = () => {
      const newCert = {
          id: `cert-${Date.now()}`,
          name: `*.domain-${(attrs.acmCertificates?.length || 0) + 1}.com`,
          type: AcmCertificateType.PUBLIC
      };
      updateAttribute('acmCertificates', [...(attrs.acmCertificates || []), newCert]);
  };

  const removeCert = (id: string) => {
      updateAttribute('acmCertificates', attrs.acmCertificates.filter(c => c.id !== id));
  };

  const updateCert = (id: string, field: string, value: any) => {
      const updated = attrs.acmCertificates.map(c => {
          if (c.id === id) return { ...c, [field]: value };
          return c;
      });
      updateAttribute('acmCertificates', updated);
  };

  return (
    <div className="space-y-6">
        {/* 1. Certificates */}
        <Accordion title="Certificates" defaultOpen={true}>
            <div className="space-y-6">
                <div className="space-y-4">
                    {attrs.acmCertificates && attrs.acmCertificates.map((cert, index) => (
                        <div key={cert.id} className="bg-white p-5 border border-gray-200 rounded-lg shadow-sm relative group animate-in fade-in">
                            <div className="absolute top-3 right-3">
                                <button 
                                    onClick={() => removeCert(cert.id)}
                                    className="text-gray-400 hover:text-red-500 transition-colors"
                                >
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                </button>
                            </div>
                            
                            <h4 className="text-sm font-bold text-gray-700 mb-4 flex items-center">
                                <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center mr-2 text-xs">{index + 1}</span>
                                {cert.name}
                            </h4>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <AWSInput 
                                    label="Domain Name" 
                                    value={cert.name}
                                    onChange={(e) => updateCert(cert.id, 'name', e.target.value)}
                                />
                                <AWSSelect 
                                    label="Certificate Type" 
                                    value={cert.type} 
                                    onChange={(e) => updateCert(cert.id, 'type', e.target.value)}
                                >
                                    <option value={AcmCertificateType.PUBLIC}>Public (Free)</option>
                                    <option value={AcmCertificateType.PRIVATE}>Private CA (Paid)</option>
                                </AWSSelect>
                            </div>
                        </div>
                    ))}

                    <button 
                        onClick={addCert}
                        className="w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-aws-primary hover:text-aws-primary transition-colors text-sm font-medium flex items-center justify-center"
                    >
                        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                        Add Certificate
                    </button>
                </div>
            </div>
        </Accordion>

        {/* 2. Private CA */}
        <Accordion title="Private Certificate Authority">
            <div className="space-y-6">
                <FormSection 
                    label="Active Private CAs" 
                    info="Number of CAs operated"
                    tooltip="Monthly cost is incurred for each active Private CA."
                >
                    <AWSInput 
                        type="number" min={0}
                        value={attrs.acmPrivateCaCount}
                        onChange={(e) => updateAttribute('acmPrivateCaCount', Number(e.target.value))}
                        unit="CAs"
                    />
                </FormSection>
            </div>
        </Accordion>
    </div>
  );
};

export const ACMUsageConfig: React.FC<ACMModuleProps> = ({ config, onUpdate }) => {
    return (
        <div className="bg-gray-50 p-6 rounded-lg border border-gray-200 text-center text-sm text-gray-500">
            <p>ACM costs are primarily driven by the number of Private Certificates and Private CAs configured in the Architecture tab. Public certificates are free.</p>
        </div>
    );
};